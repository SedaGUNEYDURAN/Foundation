...
•	**Paralel Garbage Collector,** JVM üzerinde çalışan uygulamalarda çöp toplama işlemini birden fazla thread ile gerçekleştiren garbage collector algoritmasıdır.    

- Used heap alanındaki testere dişi deseninin alt seviyesi sürekli yükseliyor. Bu ne demekti? Gatbage collector temizliği sonrası kalan canlı very. Bu nesnelerin referanslarının kaybolmadığı ve memory leak olduğu anlamına gelir. JVM bu sızıntıyı tolare edebilmek için heap size’I sürekli büyütüyor. Yaklaşık olarak 13.40 civarında hem used heap hem de heap size belirlenen maksimum limit olan 512 MB’a dayanıyor, kriz anı. Artık genişleyecek veya da temizlenebilecek yer kalmamış.    

<img width="601" height="242" alt="image" src="https://github.com/user-attachments/assets/e9925725-37fa-4802-baf1-b4d3779f95b5" />

- Burada turuncu alan CPU kullanımını gösteriyor. Mavi alan ise CPU’nun sadece çöp toplama için harcadığı eforu gösteriyor. 13.36 itibari ile garbage collector activity artan bellek kullanımı ile artmaya başlamış. JVM zorlanıyor yani. 13.40’a baktığımızda CPU kullanımı %100’e fırlıyor. Bu %100’lük kullanımın tamamı GC acticity tarafından ele geçirilmiş durumda. JVM yer almak için var gücüyle garbage collector çalıştırdığı ama leak nedeniyle başarılı olamadığı garbage collector thrashin durumudur.   

  <img width="602" height="244" alt="image" src="https://github.com/user-attachments/assets/6ca7c1a2-522b-4a70-ab59-c2a404d07456" />

- Grafik gözlem süreci boyunca doğrusal bir artış göstermiş. Canlı thread sayısı 4100 üzerine çıkmış. Burada bir thread leak olduğunu açıkça görüyoruz. Yani uygulama sürekli olarak yeni threadler yaratıyor ve bunları sonlandırmayı unutuyor.
  
  <img width="602" height="240" alt="image" src="https://github.com/user-attachments/assets/9cdc5ccd-a421-45e8-9503-6972bb1ecaa3" />

-  Grafiği incelediğimizde yüklenen class sayısının sabit kaldığını gözlemliyoruz. Yani sorunumuz dinamik sınıf yüklenmesinden kaynaklanmıyor. Bu açıdan bu sorunumuz yok.

<img width="602" height="245" alt="image" src="https://github.com/user-attachments/assets/a85d092d-56b8-473f-b717-61bab0836b90" />

- Kısaca uygulamada thread lead var, sürekli olarak yeni threadler açılıp kapatılmıyor. Bu binlerce thread kendi kullandıkları nesneleri referans olarak tutmaya devam ediyor. Bu durumda garbage collectorün bu nesneleri temizlemisini engelleyerek memory leak oluşmasına yol açıyor. Memory leak maksimum 512 MB’lık alanı doldurunca system Garbage Collector Trashing’e giriyor ve uygulama kilitleniyor. Garbage Collector Thrashing nedir dersek; JVM’in uygulamayı çalıştırmak yerine sürekli çöp toplama işlemi yapmasıdır.  Bu da uygulamanın çalışmasını engeller ve system kaynaklarını tüketir.

•İki farklı garbage collector kullanımını inceledik, peki buradan ne çıkarttık? Uygulamada bir thread leak ya da memory leak varsa hangi garbage collector kullandığımızın bir önemi yoktur. Garbage collector değiştirmek sadece uygulamanın ne zaman ve nasıl çökeceğini değiştirir. Çöküş kaçınılmazdır. Çözüm ise bu profilleme aracılığı ile sızıntının nedenini bulup düzeltmektir.    
•	İncelediğimiz kodu çalıştırdığımızda **OutOfMemoryError** exceptionını alırız.   
